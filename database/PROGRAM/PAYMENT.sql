...
--
CREATE OR REPLACE PROCEDURE READ_ALL_PAYMENT
IS
...
CREATE OR REPLACE PROCEDURE READ_AVAILABLE_PAYMENT
IS
    L_CURSOR SYS_REFCURSOR;
BEGIN
    OPEN L_CURSOR FOR
...
CREATE OR REPLACE FUNCTION DELETE_A_PAYMENT(L_PAYMENT_ID IN INTEGER) RETURN INTEGER
AS
BEGIN
    DELETE FROM PAYMENT
    WHERE PAYMENT.ID = L_PAYMENT_ID;

    RETURN SQL%ROWCOUNT;  
END;
/

CREATE OR REPLACE FUNCTION CREATE_PAYMENT(L_METHOD IN STRING, L_NUMBER IN STRING, L_STATUS IN INTEGER) RETURN INTEGER
AS
...
BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            INSERT INTO PAYMENT(ID, METHOD, ACC_NUMBER, STATUS)
            VALUES (NULL, L_METHOD, L_NUMBER, L_STATUS);
            RETURN 0;
END;
/
