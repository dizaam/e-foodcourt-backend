...
BEGIN
    ...
    INSERT INTO INVOICE(ID, BILL, FEE, CUSTOMER_ID, PAYMENT_ID)
    VALUES (INVOICE_SEQ.NEXTVAL, 0, 0.1, L_CUSTOMER_ID, L_PAYMENT_ID);
    ...
    FOR L_MERCHANT IN (SELECT DISTINCT MERCHANT_ID FROM (SELECT MERCHANT_ID FROM (SELECT * FROM CART_ITEM WHERE CART_ID = L_CART_ID) L_CART_ITEM INNER JOIN PRODUCT ON PRODUCT.ID = L_CART_ITEM.PRODUCT_ID)) LOOP
        INSERT INTO ORDERS(ID, TOTAL_ITEM, TOTAL_PRICE, CUSTOMER_ID, MERCHANT_ID, INVOICE_ID)
        VALUES(ORDER_SEQ.NEXTVAL, 0, 0, L_CUSTOMER_ID, L_MERCHANT.MERCHANT_ID, INVOICE_SEQ.CURRVAL);
