...
---
CREATE OR REPLACE PROCEDURE READ_A_PRODUCT(L_PRODUCT_ID IN INTEGER) IS
...
CREATE OR REPLACE PROCEDURE READ_PRODUCT_BYMERCHANT(L_MERCHANT_ID IN INTEGER) IS
    L_CURSOR SYS_REFCURSOR;
BEGIN
    OPEN L_CURSOR FOR
        SELECT *
...
CREATE OR REPLACE FUNCTION DELETE_A_PRODUCT(L_PRODUCT_ID IN INTEGER) RETURN INTEGER
AS
BEGIN
    DELETE FROM PRODUCT
    WHERE PRODUCT.ID = L_PRODUCT_ID;

    RETURN SQL%ROWCOUNT;  
END;
/

CREATE OR REPLACE PROCEDURE BOUGHT_PRODUCT(L_CUSTOMER_ID IN INTEGER)
IS
    L_CURSOR SYS_REFCURSOR;
BEGIN
    OPEN L_CURSOR FOR
        SELECT PRODUCT_ID
        FROM JOINED_ORDERS_ORDERS_ITEM
